...
AIチェックは既定で自動実行されます。Codex CLIはnpmの標準配置、Codexデスクトップアプリのインストール先、PATH上の`codex.exe`の順に自動検出します。
...
候補タブの上段は、条件を通過した `Entry` 候補だけを方向別に並べた一覧です。行を1件選ぶと、下段の「候補詳細」にその行の根拠とAIチェック結果が表示されます。候補は注文・約定ではなく、次の取引セッションに向けて利用者が確認するための参考情報です。

| 項目 | 見るポイント |
| --- | --- |
| コード / 銘柄名 | 証券会社画面で確認する対象銘柄の識別に使います。類似名やコード入力間違いを防ぐため、発注前に必ず証券会社側でも照合します。 |
| Long/Short | `Long` は信用買いの新規候補、`Short` は信用売りの新規候補です。実際に売建できるか、銘柄規制がないか、必要証拠金が足りるかは別途証券会社で確認します。 |
| スコア | 条件一致の強さによる候補内の相対順位です。勝率、期待利益、確実性、推奨度ではありません。高スコアでも損失はあり得ます。 |
| 最終確定終値 | 分析で使った最新の確定日足の終値です。約定価格ではありません。手動約定登録時に、この値やサイン日時を自動採用してはいけません。 |
| 主な判定理由 | MACD、EMA、出来高、ATRを組み合わせた条件の要約です。詳細は行を選択して下段を確認します。 |
| AI状態 / AI Verdict / 候補方向との関係 | AIチェックの処理状況と、AIの見解が候補方向と整合・逆・中立のどれかを示します。AI結果だけで売買を決めません。 |

#### 候補詳細の読み方

行を選ぶと、左側の「候補の根拠」に次を表示します。

- `判定基準バー日`: テクニカル判定に使った最新確定日足の日付です。更新を実行した日時とは別です。
- `Entry種別`: 初期リリースでは新規候補だけを表示します。保有中の損切・利確・継続判断は「保有」タブで確認します。
- `適用戦略`と`信頼度`: 保存済みの戦略とスコア帯です。設定や現在の相場に合わせて再解釈せず、その判定時点の情報として読みます。
- `主な判定理由`: `新規に条件成立`は前日まで不一致だった条件が当日に成立したこと、`条件を継続`はすでに成立していた条件が継続したことを表します。どちらも売買の必要性や利益を保証しません。

主な判定理由には、方向に応じたMACDの位置関係、EMA20・EMA50・EMA200の厳密な並び、出来高倍率、ATRが表示されます。単一項目だけで判断せず、出来高の一時的な急増、決算予定、地合い、信用取引の可否・規制、許容損失額を利用者自身で追加確認してください。

右側の「AIチェックの状態・結果」では、AIによる要約とVerdictを確認できます。`情報不足`は中立ではなく、判断材料が不足している状態です。`失敗`や`timeout`でも候補そのものは無効になりませんが、AIの裏付けが得られていないことを意味します。

#### 操作の意味

- 行内の`実行`、または下段右上の`選択した候補をAIチェック`は、選択した候補をAIチェックのキューへ追加します。複数行選択もできます。注文・約定は行いません。
- `取消`は待機中のAIチェックだけを取り消します。`再試行`は失敗、timeout、情報不足、キャンセル済みの試行に対して新しい試行を追加します。過去結果は上書きされません。
- `詳細`は保存済みのAI結果を確認するためのものです。候補方向とAI Verdictが逆の場合も、自動的に候補を削除したり決済したりしません。
- `約定を手入力`は、銘柄・方向を入力補助として登録画面へ渡すだけです。証券会社で実際に約定した後に、利用者が日時・価格・株数を確認して入力します。

#### AIチェックを詳しく読む

AIチェックは、テクニカル条件で抽出済みの候補について、ニュース、決算・業績、バリュエーション、配当、業種・相場環境、イベント、流動性などに追加の注意点がないかを調べる補助です。テクニカル判定を置き換えるものではなく、注文を作成・送信したり、約定を登録したりしません。

既定では日次更新のあとに、`Long` と `Short` それぞれスコア上位3件が自動でキューに入ります。自動投入を無効にしている場合や、個別に調べ直したい場合は、候補を選択して `AIチェック` を押します。処理は日次更新とは別にバックグラウンドで進み、候補一覧とAIカードは約5秒ごとに更新されます。

| AI状態 | 意味と利用者の対応 |
| --- | --- |
| 未実行 | AIチェックがまだない状態です。必要なら候補を選び、`実行`または`選択した候補をAIチェック`を押します。 |
| 待機中 | 実行待ちです。`取消`でキューから外せます。 |
| 実行中 | Codex CLIが調査中です。アプリを閉じる必要がある場合を除き、完了を待ちます。 |
| 成功 | `Verdict`、信頼度、要約が取得できました。`詳細`を開き、要約を自分の確認材料として読みます。 |
| 情報不足 | 判断に必要な情報を十分に取得できなかった状態です。`Neutral（中立）`ではありません。必要なら`再試行`するか、証券会社・会社開示などで自分で確認します。 |
| 失敗 / timeout | CLI起動、ログイン、ネットワーク、応答形式、または制限時間で完了しなかった状態です。テクニカル候補は残りますが、AIによる追加確認は未完了です。原因を確認してから`再試行`します。 |
| キャンセル | 待機中の処理を取り消した状態です。必要なら新しく実行します。 |
| 旧結果 | 以前の分析時点に対する結果です。履歴としては残りますが、現在の候補判断には使いません。最新の候補に対して再実行します。 |

`Verdict` はAIによる相場見通しで、`Bullish`（強気）、`Neutral`（中立）、`Bearish`（弱気）のいずれかです。売買推奨や利益見込みではありません。`候補方向との関係`は、`Long + Bullish`、または`Short + Bearish`を「候補方向と整合」、反対の組み合わせを「候補方向と逆」、`Neutral`を「中立」と表示します。整合していても取引すべきという意味ではなく、逆でも候補を自動削除・決済しません。

`信頼度`はAI結果の確実性を保証する数値ではなく、`High`、`Medium`、`Low`というラベルです。`詳細`には、保存済みの状態、Verdict、候補方向との関係、AI要約を表示します。AI要約は、評価時点で得られた材料に基づく参考情報として扱い、発注前には価格、売建可否、規制、決算予定、許容損失額を証券会社・公式開示等で確認してください。

AIが毎回開始できない場合は、CodexデスクトップアプリまたはCLIへのログイン、ネットワーク接続、`SWING_ADVISER_CODEX_PATH`の指定を確認します。通常の応答待ちが長い場合は、既定300秒の`SWING_ADVISER_CODEX_TIMEOUT_SECONDS`を必要に応じて調整できます。設定値の一覧と設定方法は [`runtime-configuration.md`](./runtime-configuration.md) を参照してください。

#### 候補が0件の場合

候補が0件はエラーとは限りません。条件に合う銘柄がないほか、履歴不足、確定日足の欠損、企業アクションの要照合、取得失敗でスキャン対象から除外されている場合があります。更新カードの失敗件数と、判定基準日が最新確定日足になっているかを確認してください。候補がないことを、無理に条件を緩めて埋める理由にはしません。
