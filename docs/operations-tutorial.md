...
3. 進捗カードで、実行中のステップ名、完了ステップ数、銘柄取得件数、成功数・失敗数、経過時間を確認します。「外部データを更新」では評価日の確定足と分析窓がキャッシュにある銘柄は通信せず、不足分だけを取得します。ネットワーク待機中はバーが動き、詳細に待機中の取得元と経過時間を表示します。
...
復元は、アプリを閉じた状態で現在のDBを別名へ退避してから、バックアップファイルを同じ名前・同じexeフォルダへ戻します。ただし、復元手順の実機テストはPhase 13の未完了項目です。初回の本番利用前に、コピーしたバックアップを別フォルダで確認する運用を推奨します。
