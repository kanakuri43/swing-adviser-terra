...
日次更新の判定基準日は、JSTの平日15:30以降は当日、それより前と週末は直近平日です。したがって、15:30以降に更新した当日の候補・保有判定は翌営業日の判断材料として表示します。この時刻条件だけで当日足を確定扱いにはしません。取得した日足が `Final` または `Corrected` でない場合、祝日などで当日足がない場合は、既存のpoint-in-time検証が候補と保有判定を fail-closed で除外し、その理由を記録します。
